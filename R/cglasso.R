...
#' \deqn{D^{-1/2} \left( \arg\min_{\Theta} \operatorname{Tr}[\hat{R} \hat{\Theta}] - \log \det \Theta + \sum_{i \ne j} |\Theta_{ij}| \right) D^{-1/2}} for the given D. If type is \code{II}, the algorithm solves CGLASSO-II in the reference. It is for each iterative classo with covariate update, the squared-root of scale matrix D^{-1/2} is multiplied. Please refer to the equation (5.2) in the reference for the details.

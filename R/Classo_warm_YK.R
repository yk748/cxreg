#' classo_warm
#'
#' cglasso with warm start
#' @param X input1
#' @param Y input2
#' @param lambda.path input3
#' @param b.init input4
#' @return output
#' @examples
#' example <- classo_warm (X, Y, lambda.path)
#'
#' @export
classo_warm <- function(X, Y, lambda.path, b.init = NULL){

  L <- length(lambda.path)
    ...
    if (e.old > tol){
      ...
      for (j in 1:p){
        ...
        if (X.rj.l2 < lambda){
          ...
          b[c(j,j+p)] <- X.rj*(1 - lambda/X.rj.l2)

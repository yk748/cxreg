cvtype <- function(type.measure = "mse", subclass = "classofit") {
  ...
  devname  <- switch(subclass, classofit = "Mean-Squared Error")
  typenames <- c(deviance = devname, mse = "Mean-Squared Error")
  
  # For classofit only mse and deviance (indices 1 and 2) are available
  subclass.ch   <- switch(subclass, classofit = c(1, 2))
